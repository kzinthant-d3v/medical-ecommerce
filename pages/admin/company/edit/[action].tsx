...
export default function EditCategory(): JSX.Element {
  ...
  const [title, setTitle] = useState('ကုမ္ပဏီ အသစ်ထည့်ရန်');
  ...
  async function handleClick(): Promise<void> {
    if (router.query.action === 'add') {
      setLoading(true);
      await mutate(`${process.env.NEXT_PUBLIC_HOSTNAME}/api/companies`, 'POST', {
        catName: input,
      });
      router.push('/admin/company');
    } else {
      setLoading(true);
      await mutate(`${process.env.NEXT_PUBLIC_HOSTNAME}/api/companies`, 'PUT', {
        newName: input,
        id: router.query.id,
      });
      router.push('/admin/company');
  ...
  useEffect(() => {
    router.query.action === 'change'
      ? (setTitle('ကုမ္ပဏီအမည် ပြောင်းရန်'),
          ...
          <>
            ...
            <div style={{ textAlign: 'center', marginTop: '60px' }}>
              <TextArea
                ...
                style={{ width: '500px' }}
                placeholder="ကုမ္ပဏီအမည် ထည့်ပါ"
                autoSize
              />
              <Button
                type="primary"
              ...
              <Button onClick={() => router.push('/admin/company')} danger>
                နောက်သို့{' '}
              </Button>
            </div>
          </>
